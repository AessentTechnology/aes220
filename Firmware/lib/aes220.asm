...
; This file was generated Fri Mar 29 21:50:52 2013
;--------------------------------------------------------
	.module aes220
...
;	aes220.c:143: delay(200); // 200ms increased from 100ms or aes220b uC hangs
	mov	dptr,#0x00C8
	lcall	_delay
;	aes220.c:146: if (DONE == 1) { fpgaProgd = TRUE; }
	jnb	_PD1,00108$
	mov	r2,#0x01
	sjmp	00109$
00108$:
;	aes220.c:147: else { fpgaProgd = FALSE; } // Blank Flash or problem during config
	mov	r2,#0x00
00109$:
;	aes220.c:151: OEA = 0x00; // Port A as input
	mov	_OEA,#0x00
;	aes220.c:152: OEB = 0x00; // Port B as input
	mov	_OEB,#0x00
;	aes220.c:153: OED = 0x81; // Port D as input save for PROG_B and SUSPEND_F
	mov	_OED,#0x81
;	aes220.c:154: PROG_B = 1;
	setb	_PD0
;	aes220.c:155: SUSPEND_F = 0;
	clr	_PD7
;	aes220.c:157: return fpgaProgd;
	mov	dpl,r2
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	aes220.c:160: BYTE setupFpgaConf() 
;	-----------------------------------------
;	 function setupFpgaConf
;	-----------------------------------------
_setupFpgaConf:
;	aes220.c:165: OED = 0xC5; // set PD0/2/6/7 as outputs, the rest as inputs
	mov	_OED,#0xC5
;	aes220.c:166: SUSPEND_F = 0;
	clr	_PD7
;	aes220.c:167: PROG_B = 0; // Reset the FPGA
	clr	_PD0
;	aes220.c:168: OEA = 0xFF; // set port A as outputs (SelectMAP data bus)
	mov	_OEA,#0xFF
;	aes220.c:169: OEB = 0xFF; // set port B as outputs
	mov	_OEB,#0xFF
;	aes220.c:171: INIT_B = 1;
	setb	_PD5
;	aes220.c:172: RDWRB  = 1;
	setb	_PB4
;	aes220.c:173: CSI_B  = 1;    // Chip select signal set permanently to 0 (one 
	setb	_PD2
;	aes220.c:175: CCLK  = 0;    // sets CCLK low
	clr	_PD6
;	aes220.c:177: M2    = 1;    // M2 = 1 for SelectMap mode 
	setb	_PB1
;	aes220.c:178: M1    = 1;    // M1 = 1 for SelectMap mode
	setb	_PB5
;	aes220.c:179: M0    = 0;    // M0 = 0 for SelectMap mode
	clr	_PB3
;	aes220.c:182: delay(500); // 500ms
	mov	dptr,#0x01F4
	lcall	_delay
;	aes220.c:183: while (INIT_B != 0) {}; // wait for INIT_B to go low
00101$:
	jb	_PD5,00101$
;	aes220.c:184: PROG_B = 1;             // PROG_B back up
	setb	_PD0
;	aes220.c:185: while (INIT_B != 1) {}; // Wait for INIT_B to do the same
00104$:
	jnb	_PD5,00104$
;	aes220.c:186: RDWRB = 0; // select write mode
	clr	_PB4
;	aes220.c:187: CSI_B = 0;  // assert chip select
	clr	_PD2
;	aes220.c:188: delay(500);
	mov	dptr,#0x01F4
	lcall	_delay
;	aes220.c:189: if (DONE == 0) {fpgaStatus = F_READY;}
	jb	_PD1,00108$
	mov	r2,#0xAD
	sjmp	00109$
00108$:
;	aes220.c:190: else fpgaStatus = F_NOT_READY;
	mov	r2,#0xB3
00109$:
;	aes220.c:191: return fpgaStatus;
	mov	dpl,r2
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	aes220.c:194: BYTE configureFpga(DWORD dataLen)
;	-----------------------------------------
...
;	aes220.c:199: fpgaStatus = setupFpgaConf();
	lcall	_setupFpgaConf
	mov	r6,dpl
;	aes220.c:201: if (fpgaStatus == F_READY) {
	cjne	r6,#0xAD,00127$
	sjmp	00128$
00127$:
	ljmp	00112$
00128$:
;	aes220.c:202: while(dataLen > 0) { 
00105$:
	...
	jz	00107$
;	aes220.c:203: if ( !(EP2468STAT & bmEP2EMPTY) ) { // EP2 received data
	mov	a,_EP2468STAT
	jb	acc.0,00105$
;	aes220.c:207: bytes = MAKEWORD(EP2BCH,EP2BCL);
	mov	dptr,#_EP2BCH
	movx	a,@dptr
	mov	r0,a
	mov	r7,#0x00
...
;	aes220.c:209: for (i=0;i<bytes;++i) {
	mov	r2,#0x00
	mov	r3,#0x00
00113$:
	clr	c
	...
	jnc	00116$
;	aes220.c:210: IOA = EP2FIFOBUF[i]; // output the byte on port A
	mov	dpl,r2
	mov	a,#(_EP2FIFOBUF >> 8)
	add	a,r3
	mov	dph,a
	movx	a,@dptr
	mov	_IOA,a
;	aes220.c:211: CCLK = 0;     // tick the clock (low)
	clr	_PD6
;	aes220.c:212: LED6 = 1;     // flash the LED, why not?
	setb	_PB0
;	aes220.c:213: CCLK = 1;     // tick the clock (high)
	setb	_PD6
;	aes220.c:214: LED6 = 0;     // keep flashing this LED
	clr	_PB0
;	aes220.c:209: for (i=0;i<bytes;++i) {
	inc	r2
	cjne	r2,#0x00,00113$
...
;	aes220.c:216: dataLen -= bytes;
	...
	subb	a,r5
	mov	(_configureFpga_dataLen_1_1 + 3),a
;	aes220.c:217: if ( (INIT_B == 0) & (DONE == 0) ) {
	mov	c,_PD5
	cpl	c
	clr	a
	rlc	a
	...
	anl	a,r2
	jz	00102$
;	aes220.c:219: LED6 = 1; // turn LED off
	setb	_PB0
;	aes220.c:220: return fpgaStatus;
	mov	dpl,#0xB0
	ret
00102$:
;	aes220.c:223: REARM();
	...
	ljmp	00105$
00107$:
;	aes220.c:227: if (DONE == 1) {
	jnb	_PD1,00109$
;	aes220.c:229: OEA = 0x00; // Port A as input
	mov	_OEA,#0x00
;	aes220.c:230: assertSoftReset(); // keep soft_reset at one (active)
	lcall	_assertSoftReset
;	aes220.c:232: CSI_B = 1;  // release chip select
	setb	_PD2
;	aes220.c:233: RDWRB = 1; // release write mode
	setb	_PB4
;	aes220.c:236: OEB = 0x00; // Port B as input
	mov	_OEB,#0x00
;	aes220.c:237: OED = 0x81; // Port D as input save for PROG_B and SUSPEND_F
	mov	_OED,#0x81
;	aes220.c:238: fpgaStatus = PROG_DONE;
	mov	r6,#0xAB
	sjmp	00112$
00109$:
;	aes220.c:240: else fpgaStatus = DONE_LOW;
	mov	r6,#0xB2
00112$:
;	aes220.c:242: return fpgaStatus;
	mov	dpl,r6
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;------------------------------------------------------------
;	aes220.c:245: void setupFpgaProg() 
;	-----------------------------------------
;	 function setupFpgaProg
;	-----------------------------------------
_setupFpgaProg:
;	aes220.c:250: OEA = 0x00; // set port A as inputs
	mov	_OEA,#0x00
;	aes220.c:251: OEB = 0xFF; // set port B as outputs
	mov	_OEB,#0xFF
;	aes220.c:252: OED = 0b10001101; // set PD0, PD2, PD3 and PD7 as outputs
	mov	_OED,#0x8D
;	aes220.c:253: SUSPEND_F = 0;
	clr	_PD7
;	aes220.c:255: PROG_B = 1;
	setb	_PD0
;	aes220.c:256: LOAD_B = 1;    // Shift register load signal set to 1
	setb	_PD2
;	aes220.c:257: CS_B   = 1;    // Chip select signal set to 1
	setb	_PD3
;	aes220.c:258: fpgaStatus = PROG_F_MODE;
...
;------------------------------------------------------------
;	aes220.c:261: BYTE progFpga()
;	-----------------------------------------
;	 function progFpga
;	-----------------------------------------
_progFpga:
;	aes220.c:265: setupFpgaProg();
	lcall	_setupFpgaProg
;	aes220.c:266: if (fpgaStatus != PROG_F_MODE) return 1;
	mov	dptr,#_fpgaStatus
	movx	a,@dptr
	mov	r2,a
	cjne	r2,#0xA8,00145$
...
00145$:
	mov	dpl,#0x01
	ret
;	aes220.c:267: while(fpgaStatus != PROG_DONE) { 
00111$:
	mov	dptr,#_fpgaStatus
	movx	a,@dptr
	mov	r2,a
	...
	ljmp	00113$
00146$:
;	aes220.c:268: if ( !(EP2468STAT & bmEP2EMPTY)) {
	mov	a,_EP2468STAT
	jb	acc.0,00111$
;	aes220.c:272: bytes = MAKEWORD(EP2BCH,EP2BCL);
	mov	dptr,#_EP2BCH
	movx	a,@dptr
	mov	r3,a
	mov	r2,#0x00
...
;	aes220.c:274: ctrlCode = EP2FIFOBUF[0];
	mov	dptr,#_EP2FIFOBUF
	movx	a,@dptr
	mov	r2,a
;	aes220.c:275: switch (ctrlCode) {
	cjne	r2,#0x60,00148$
	sjmp	00103$
00148$:
	cjne	r2,#0x61,00149$
...
00151$:
	ljmp	00107$
;	aes220.c:276: case STATUS_CHECK:
00103$:
;	aes220.c:277: CS_B = 0;    // Enable flash
	clr	_PD3
;	aes220.c:278: LOAD_B = 0;
	clr	_PD2
;	aes220.c:279: IOB = 0xD7;  // Write the value on port B
	mov	_IOB,#0xD7
;	aes220.c:280: LOAD_B = 1;
	setb	_PD2
;	aes220.c:281: IOB = 0x55;  // Write a dummy value to flush out status byte from flash
	mov	_IOB,#0x55
;	aes220.c:282: LOAD_B = 0;
	clr	_PD2
;	aes220.c:283: LOAD_B = 1;
	setb	_PD2
;	aes220.c:284: EP6FIFOBUF[0] = IOA; // read and send value on port A back
	mov	dptr,#_EP6FIFOBUF
	mov	a,_IOA
	movx	@dptr,a
;	aes220.c:285: CS_B = 1;
	setb	_PD3
;	aes220.c:287: EP6BCH=0;
	mov	dptr,#_EP6BCH
	clr	a
	movx	@dptr,a
;	aes220.c:288: SYNCDELAY();
	 nop 
	 nop 
	 nop 
	 nop 
;	aes220.c:289: EP6BCL=1;
	mov	dptr,#_EP6BCL
	mov	a,#0x01
	movx	@dptr,a
;	aes220.c:290: break; // end of case STATUS_CHECK
	ljmp	00108$
;	aes220.c:291: case WRITE_CMD:
00104$:
;	aes220.c:292: CS_B = 0;
	clr	_PD3
;	aes220.c:293: for (i=1;i<bytes;++i) {
	mov	r2,#0x01
	mov	r3,#0x00
00114$:
	clr	c
	...
	jnc	00117$
;	aes220.c:294: LOAD_B = 0;
	clr	_PD2
;	aes220.c:295: IOB = EP2FIFOBUF[i];
	mov	dpl,r2
	mov	a,#(_EP2FIFOBUF >> 8)
	add	a,r3
	mov	dph,a
...
;	aes220.c:298: CS_B = 1;
	setb	_PD3
;	aes220.c:299: break; // end of case WRITE_CMD
;	aes220.c:300: case READ_CMD:
	sjmp	00108$
00105$:
;	aes220.c:301: CS_B = 0;
	clr	_PD3
;	aes220.c:302: for (i=1;i<bytes;++i) {
	mov	r2,#0x01
	mov	r3,#0x00
00118$:
	clr	c
	...
	subb	a,(_progFpga_bytes_1_1 + 1)
	jnc	00121$
;	aes220.c:303: LOAD_B = 0;
	clr	_PD2
;	aes220.c:304: IOB = EP2FIFOBUF[i];
	mov	dpl,r2
	mov	a,#(_EP2FIFOBUF >> 8)
	add	a,r3
	mov	dph,a
...
;	aes220.c:302: for (i=1;i<bytes;++i) {
	...
	sjmp	00118$
00121$:
;	aes220.c:307: for (i=0;i<PAGE_SIZE;++i) {
	mov	r2,#0x00
	mov	r3,#0x00
00122$:
	clr	c
	...
	subb	a,#0x01
	jnc	00125$
;	aes220.c:308: LOAD_B = 0;
	clr	_PD2
;	aes220.c:309: IOB = 0xAA; // dummy byte
	mov	_IOB,#0xAA
;	aes220.c:310: LOAD_B = 1;
	setb	_PD2
;	aes220.c:311: EP6FIFOBUF[i] = IOA;
	mov	dpl,r2
	mov	a,#(_EP6FIFOBUF >> 8)
	add	a,r3
	mov	dph,a
...
;	aes220.c:313: CS_B = 1;
	setb	_PD3
;	aes220.c:315: EP6BCH=MSB(PAGE_SIZE);
	mov	dptr,#_EP6BCH
	mov	a,#0x01
	movx	@dptr,a
;	aes220.c:316: SYNCDELAY();
	 nop 
	 nop 
	 nop 
	 nop 
;	aes220.c:317: EP6BCL=LSB(PAGE_SIZE);
	mov	dptr,#_EP6BCL
	mov	a,#0x08
	movx	@dptr,a
;	aes220.c:318: break; // end of case READ_CMD
;	aes220.c:319: case RESET_F:
	sjmp	00108$
00106$:
;	aes220.c:320: startFpga();
	lcall	_startFpga
;	aes220.c:321: fpgaStatus = PROG_DONE;
	mov	dptr,#_fpgaStatus
	mov	a,#0xAB
	movx	@dptr,a
